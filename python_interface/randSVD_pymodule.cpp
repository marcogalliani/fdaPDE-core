...
PYBIND11_MODULE(randSVD,m){
    ...
    py::class_<d_RSI, d_RSVD>(m, "RSI")
        ...
        .def("compute", pybind11::overload_cast<const DMatrix<double>&,int,int>(&d_RSI::compute));

    using d_RBKI = RBKI<DMatrix<double>>;
    py::class_<d_RBKI , d_RSVD>(m, "RBKI")
        .def(py::init<double,int,unsigned int>())
        .def("compute", pybind11::overload_cast<const DMatrix<double>&,int,int>(&d_RBKI::compute));

    using d_GenRSI = GeneralizedRSI<DMatrix<double>>;
    py::class_<d_GenRSI, d_RSVD>(m, "GeneralizedRSI")
        .def(py::init<double,int,unsigned int>())
        .def("compute", pybind11::overload_cast<const DMatrix<double>&,int,int>(&d_GenRSI::compute));

    using d_GenRBKI = GeneralizedRBKI<DMatrix<double>>;
    py::class_<d_GenRBKI , d_RSVD>(m, "GeneralizedRBKI")
        .def(py::init<double,int,unsigned int>())
        .def("compute", pybind11::overload_cast<const DMatrix<double>&,int,int>(&d_GenRBKI::compute));
    ...
    py::class_<d_NysRSI, d_REVD>(m, "NysRSI")
        .def(py::init<double,int,unsigned int>())
        .def("compute", pybind11::overload_cast<const DMatrix<double>&,int,int>(&d_NysRSI::compute));

    using d_NysRBKI = NysRBKI<DMatrix<double>>;
    py::class_<d_NysRBKI, d_REVD>(m, "NysRBKI")
        .def(py::init<double,int,unsigned int>())
        .def("compute", pybind11::overload_cast<const DMatrix<double>&,int,int>(&d_NysRBKI::compute));
}
